...
const Signup: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:3001/api/auth/signup', { email, password, figmaToken });
    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <div className="bg-[#111] p-8 rounded-2xl border border-[#222] w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[11px] font-bold text-[#555] uppercase tracking-widest">Figma Token (Optional)</label>
            <input
              type="password"
              value={figmaToken}
